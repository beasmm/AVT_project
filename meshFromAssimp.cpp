...
bool Import3DFromFile(const std::string& filePath, Assimp::Importer& importer, const aiScene*& scene, float& scaleFactor)
{
	// Use Assimp to import the 3D model from the specified file
	scene = importer.ReadFile(filePath,
		aiProcess_Triangulate |        // Triangulates all faces
		aiProcess_FlipUVs |            // Flips UV coordinates to match OpenGL's coordinate system
		aiProcess_CalcTangentSpace |   // Calculates tangents and bitangents
		aiProcess_GenSmoothNormals |   // Generates smooth normals
		aiProcess_JoinIdenticalVertices | // Optimizes mesh data
		aiProcess_ImproveCacheLocality | // Optimize for post-transform vertex cache
		aiProcess_LimitBoneWeights |   // Limits bone weights to 4 per vertex
		aiProcess_RemoveRedundantMaterials | // Removes redundant materials
		aiProcess_SortByPType);        // Splits meshes by primitive type
	...
	// Calculate the largest dimension to determine the scaling factor
	float maxDimension = std::max({ scene_max.x - scene_min.x, scene_max.y - scene_min.y, scene_max.z - scene_min.z });
	scaleFactor = 1.0f / maxDimension;

	// All done, the scene will be cleaned up automatically when 'importer' goes out of scope
	return true;
}
